...
def test_odds_ratio_config(level_1: Union[str, int], level_2: Union[str, int]):
    ...
    """
    bn, _ = _test_bn(level_1=level_1, level_2=level_2)
    config = {
        "target_node": "A",
    ...
    ors = bn.adjusted_odds_ratio(config=config)
...
def test_odds_ratio_all(level_1: Union[str, int], level_2: Union[str, int]):
    ...
    Test both string and int input data.
    """
    bn, data = _test_bn(level_1=level_1, level_2=level_2)
    ors = bn.adjusted_odds_ratio(target="A")
    assert all([isinstance(odds_ratio, float) for key, odds_ratio in ors.items()])


@pytest.mark.parametrize("level_1,level_2", [(0, 1), ("level_1", "level_2")])
def test_odds_ratio_cpdag(level_1: Union[str, int], level_2: Union[str, int]):
    """
    Test both string and int input data.
    """
    bn, data = _test_bn(level_1=level_1, level_2=level_2)
    ors = bn.adjusted_odds_ratio(target="A", cpdag=True, data=data)
    assert all([isinstance(odds_ratio, dict) for key, odds_ratio in ors.items()])
    assert all(
        [
            odds_ratio["min"] <= odds_ratio["median"] <= odds_ratio["max"]
            for key, odds_ratio in ors.items()
        ]
    )
